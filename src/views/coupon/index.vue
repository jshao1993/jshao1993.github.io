<template>
  <div class="app-container">
    ...
    <el-row :gutter="12">
      <el-col v-for="card in list" :key="card.id" :span="6">
        <el-card shadow="hover">
          <el-divider class="card-tip">{{ typeMapOptionsObj[card.type] }}</el-divider>
          <div class="card-wrap">
    ...
    >
      ...
      >
        ...
        <el-form-item label="优惠券名称:" prop="name">
          <el-input v-model="temp.name" disabled clearable placeholder="优惠券名称" />
        </el-form-item>
        ...
        <el-form-item
          ...
          :rules="[
            ...
            { pattern: patterns.positiveNumberByDiscount, message: '请输入1-99的正整数', trigger: 'blur' }
          ]"
          label="打折力度:"
          prop="benefit"
        >
        ...
        <el-form-item
          ...
          :rules="[
            ...
            { pattern: patterns.positiveNumber, message: '请输入正整数', trigger: 'blur' }
          ]"
          label="抵价额:"
          prop="benefit"
        >
    ...
    <div style="display: none">{{ couponName }}</div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    couponName() {
      // type---0(抵价优惠券)1(折扣优惠券)2(赠送优惠券)
      const action1 =
        +this.temp.type === 0 ? '抵' : +this.temp.type === 1 ? '打' : '赠送'
      const action2 =
        +this.temp.type === 0 ? '元' : +this.temp.type === 1 ? '折' : ''
      const part1 = this.temp.amount ? `消费${this.temp.amount}` : ''
      const benefit =
        +this.temp.type === 0
          ? +this.temp.benefit // 非数字的时 +this.temp.benefit = NaN
          : +this.temp.type === 1
            ? +this.temp.benefit / 10
            : this.temp.benefit
      const part2 = benefit ? `${action1}${benefit}${action2}` : ''
      return this.resetCouponName(part1, part2, benefit)
    }
  },
  created() {
    this.getList()
  ...
  methods: {
    ...
    handleUpdate(card) {
      ...
      this.deepCloneTemp = deepClone(this.temp)
    ...
    typeChange(val) {
      if (this.dialogStatus === 'update') {
        this.temp.benefit =
          val === this.deepCloneTemp.type ? this.deepCloneTemp.benefit : ''
      }
      this.$refs['dataForm'].clearValidate()
    },
    resetCouponName(p1, p2, b) {
      this.temp.name = `${p1}${p2}`
      return this.temp.amount + b
    }
  }
}
</script>
